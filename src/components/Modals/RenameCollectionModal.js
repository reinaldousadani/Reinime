...
const RenameCollectionModal = ({ isVisible = false, header = "" }) => {
  const { state, renameCollectionById } = useContext(CollectionContext);
  const { state: collectionListPageState, setRenameCollectionModal } =
  ...
  const handleSubmit = (e) => {
    ...
    console.log(collectionListPageState);
    renameCollectionById(
      collectionName,
      collectionListPageState.selectedCollectionId
    );
    setRenameCollectionModal(false);
    setCollectionName("");
  };
